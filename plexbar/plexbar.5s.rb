...
class NavidromePlugin
  ...
  def now_playing
    out = []
    resp = call_rest("getNowPlaying")
    resp = resp["subsonic-response"]["nowPlaying"]["entry"] || []
    if resp.size == 0
      out = ["Nothing is playing right now"]
    else 
      out = []
      resp.each do |entry|
        mins = entry["minutesAgo"] == 1 ? "min" : "mins" 
        out << "#{entry["username"]} (#{entry["playerName"]}) - #{entry["minutesAgo"] || 0} #{mins} ago | color=white"
        out << "--\"#{entry["title"]}\" | color=white"
        out << "--from #{entry["album"]} | color=white"
        out << "--by #{entry["artist"]} | color=white"
      end
      out
    end
  end
...
output = with_captured_stdout do
  ...
  # Navidrome
  if @config["navidrome"]
    navidrome = NavidromePlugin.new(@config["navidrome"])
    puts "---"
    puts "Navidrome | image=#{NAVIDROME_ICON} href=#{@config["navidrome"]["url"]}"
    every 1, navidrome, :now_playing
  end
